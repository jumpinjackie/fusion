...
FeatureLine.prototype = {
     ...
     */
    segmentTo: function(node) {
        var margin = args.length > 1?args[1]:3;
        for (var i=0; i<this.segments.length; i++) {
            return this.segments[i].hasTo(node, margin)?this.segments[i]:null;
        }        
    },
    ...
    segmentFrom: function(node) {
        var margin = args.length > 1?args[1]:3;
        for (var i=0; i<this.segments.length; i++) {
            return this.segments[i].hasFrom(node, margin)?this.segments[i]:null;
        }
        return null
    },
...
Segment.prototype = {
    ...
    hasTo: function(node, margin) {
        return this.to.near(node, margin);
    },
    ...
    hasFrom: function(node, margin) {
        return this.from.near(node, margin);
    },
...
Node.prototype = {
    ...
    /* returns true if the supplied node is
     ...
     */
      ...
      */
    near: function(node, tolerance) {
        var minX = node.x - tolerance;
        var maxX = node.x + tolerance;
        var maxY = node.y + tolerance;
        var minY = node.y - tolerance;
        return ((this.x > minX && this.x < maxX) && (this.y > minY && this.y < maxY))?true:false;
    },
