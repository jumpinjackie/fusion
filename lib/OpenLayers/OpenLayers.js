...
OpenLayers.String = {
    ...
    translate: function(key) {
      if (!OpenLayers.Strings[OpenLayers.String.langCode]) {
        this.loadDictionary(OpenLayers.String.langCode)
      }
      
      var dictionary = OpenLayers.Strings[OpenLayers.String.langCode];
  ...
  /**
    ...
    loadDictionary: function(langCode) {
        //load up the specified dictionary
        var url = OpenLayers._getScriptLocation() + OpenLayers.String.dictionaryPath + langCode + '/strings.json';
        var options = {
          onSuccess: OpenLayers.Function.bind(this.dictionaryLoaded, this, url, langCode), 
          onFailure: OpenLayers.Function.bind(this.dictionaryFailure, this, url, langCode),
          asynchronous: false         //must be synchronous call to return control here
        };                            //or find some way to load dictiaonaries at initiliazation?
        new OpenLayers.Ajax.Request(url, options);
    },
    
    dictionaryLoaded: function(url, langCode, json) {
      eval(json.responseText);
      OpenLayers.Console.log('loaded '+langCode+' dictionary at:'+url);
    },
    
    dictionaryFailure: function(url, langCode) {
      alert('failed to loaded dictionary at:'+url+' falling back to default language');
      OpenLayers.Console.log('failed to loaded dictionary at:'+url+' falling back to default language');
